...
main_window::main_window()
{
    ...
    cursor_label = new QLabel("Ln 1, Col 1");
    word_line_label = new QLabel("Words: 0  Lines: 1");
    statusBar()->addPermanentWidget(word_line_label);
    ...
    connect(editor, &QTextEdit::cursorPositionChanged, this, [this] {
        ...
        cursor_label->setText("Ln " + QString::number(line) + ", Col " + QString::number(col));
    });
...
void main_window::setup_format_menu()
{
    ...
    connect(action_font, &QAction::triggered, this, [this] {
        bool ok = false;
        const QFont current_font = editor->currentFont();
        const QFont selected = QFontDialog::getFont(&ok, current_font, this, "Font");
        if (ok) {
...
void main_window::update_status_bar()
{
    ...
    word_line_label->setText("Words: " + QString::number(word_count) + "  Lines: " + QString::number(line_count));
}
...
void main_window::show_spell_context_menu(const QPoint& pos)
{
    ...
    if (!selected_word.isEmpty() && !checker->is_correct(selected_word.toStdString())) {
        ...
        const auto suggestions = checker->suggestions(selected_word.toStdString());
        if (suggestions.empty()) {
            auto* no_suggestions = menu->addAction("No suggestions");
            no_suggestions->setEnabled(false);
        } else {
            for (const auto& suggestion : suggestions) {
                const auto q_suggestion = QString::fromStdString(suggestion);
                auto* action = menu->addAction(q_suggestion);
                connect(action, &QAction::triggered, this, [this, cursor, q_suggestion]() mutable {
                    cursor.insertText(q_suggestion);
                    editor->setTextCursor(cursor);
                });
            }
        }
